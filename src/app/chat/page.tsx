...
function App() {
  ...
  const loadPreviousSummaries = async () => {
    if (user) {
      try {
        ...
        //
        // fetchAllUsersData(user.uid).then((all_data) => {
        //   if (all_data.length > 0) {
        //     console.log('All users data:', JSON.stringify(all_data));
        //   }
        // }).catch((error) => {
  ...
  const fetchAllUsersData = async (userId: string) => {
    try {
      const q = query(
      ...
      const usersData = querySnapshot.docs.map(doc => {
        ...
        // convert cloudfire timestamp to datetime
        const date = timestamp?.toDate();
        const formattedDate = date ? date.toLocaleDateString() : '';
        const formattedTime = date ? date.toLocaleTimeString() : '';
        const formattedDateTime = date ? `${formattedDate} ${formattedTime}` : '';

        // create individual survey fields
        const preSurveyMood = data.preSurvey?.mood ?? '';
        const preSurveyAttitude = data.preSurvey?.attitude ?? '';
        const postSurveyMood = data.postSurvey?.mood ?? '';
        const postSurveyAttitude = data.postSurvey?.attitude ?? '';
        const postSurveySatisfaction = data.postSurvey?.satisfaction ?? '';
        return {
