    ...
    <div className="flex flex-col flex-1 bg-white min-h-0 rounded-xl h-full">
        ...
        >
          {transcriptItems.map((item) => {
            ...
            if (type === "MESSAGE") {
              ...
              const bubbleBase = `max-w-lg p-3 rounded-xl ${isUser ? "bg-blue-100 text-black" : "bg-gray-100 text-black"}`;
                ...
                <div key={itemId} className={containerClasses}>
                  <div className={bubbleBase}>
                    <div className={`text-xs ${isUser ? "text-gray-500" : "text-gray-500"} font-mono`}>
                      {isUser ? timestamp : timestamp + ' - Remi'}
                    </div>
              ...
              //   </div>
              // );
              return null; // Temporarily hide breadcrumbs
            } else {
      ...
      {showTextBox && (
        <div className="p-4 flex items-center gap-x-2 flex-shrink-0 border-t border-gray-200">
          ...
          >
            <Image src="/remibot/arrow.svg" alt="Send" width={24} height={24} />
          </button>
        </div>)}
    </div>
  );
